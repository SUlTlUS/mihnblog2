...
const ENV_CONFIG_OVERRIDE_KEYS = new Set([
  'TITLE',
  'AUTHOR',
  'LINK',
  'CONTACT_EMAIL',
...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  if (ENV_CONFIG_OVERRIDE_KEYS.has(key)) {
    const envValue = ENV_CONFIG_OVERRIDES[key]
    const override =
      key === 'CONTACT_EMAIL'
        ? encodeEmail(envValue)
        : envValue
    if (hasVal(override) && override !== '') {
      return convertVal(override)
    }
  }
