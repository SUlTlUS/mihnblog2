 ...
 *
 * BUILD_PREFETCH_ENABLED            是否全量预热 block（export 默认 false；true 才开启）
 * BUILD_PREFETCH_CONCURRENCY        预热并发（默认 2；慢环境建议 1–2）
 * NOTION_BUILD_RATE_MAX_PER_MINUTE  构建时限流：每分钟最大请求数（默认 15）
 * NOTION_BUILD_RATE_MIN_INTERVAL_MS 构建时限流：相邻请求最小间隔 ms（默认 1200）
...
function getBuildPrefetchConcurrency() {
  return parsePositiveInt(process.env.BUILD_PREFETCH_CONCURRENCY, 2, {
    min: 1,
    max: 8
  })
}
...
function getNotionBuildRateMaxPerMinute() {
  return parsePositiveInt(process.env.NOTION_BUILD_RATE_MAX_PER_MINUTE, 15, {
    min: 1,
    max: 600
  })
}
...
function getNotionBuildRateMinIntervalMs() {
  return parsePositiveInt(process.env.NOTION_BUILD_RATE_MIN_INTERVAL_MS, 1200, {
    min: 0,
    max: 60_000
  })
}
