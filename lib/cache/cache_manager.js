...
function cacheReadsEnabled(force) {
  // 构建/导出阶段必须允许读取本地文件缓存。
  // 否则 Cloudflare 多个 Next worker 会在同一次构建里反复 MISS，重复请求 Notion，最终触发 429。
  if (force || isBuildPhase) return true
...
function isUsableCacheValue(data) {
  if (data == null) return false
  // 空数组也是合法缓存值，例如空分类、空标签、空结果列表。
...
export async function getOrSetDataWithCustomCache(
  ...
  if (isBuildPhase) {
    const promise = withFileLock(
      key,
      async () => {
        const doubleCheck = await getDataFromCache(key, true)
        if (doubleCheck) {
