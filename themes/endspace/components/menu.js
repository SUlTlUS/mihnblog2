...
const preferredPathOrder = ['/', '/aboutme', '/friends', '/portfolio', '/archive']

const sortPreferredMenu = menuItems =>
  menuItems
    .map((item, index) => ({ item, index }))
    .sort((a, b) => {
      const aOrder = preferredPathOrder.indexOf(a.item.path)
      const bOrder = preferredPathOrder.indexOf(b.item.path)
...
export const getEndspaceMenuItems = ({ customNav, customMenu } = {}) => {
  ...
  return sortPreferredMenu(normalizeMenu(links))
}
